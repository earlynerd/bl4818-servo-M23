...
## 2026-08-13 — Homing and strike motion have local encoder safety bounds

- **Decision:** Every accepted homing or strike sequence now accumulates absolute encoder travel in the 500 Hz strike task and latches a dedicated motor fault at one revolution (16,384 counts). Homing additionally faults after 5 seconds; strike deliberately has no independent timer because low-current motion may be slow while hazardous high-current runaway consumes the travel allowance quickly. A fault disables PWM and invalidates `homed`; `CLEAR_FAULT` does not restore readiness, so a fresh home is mandatory. A successful home compares its drum position modulo one revolution with the previous known calibration and sets a non-faulting warning at a shift of 1,024 counts (22.5 degrees). `SAVE_SETTINGS` may safely pause an idle home hold to persist that calibration, then resumes the hold.
- **Why:** Bench observation showed that a loose mallet mount can move the ball out of the drum path; without a mechanical impact, the rotor can continue through the strike state at near top speed and throw the mallet or ball. Homing had the same missing termination contract even though its low duty makes it less hazardous.
- **Accepted tradeoffs:** (1) The safety check runs only while a sequence is active and only at 500 Hz, adding no work to the 6 kHz current loop or 20 kHz PWM/ADC path; a fault can therefore occur up to one 2 ms strike tick after the exact threshold. (2) The home-shift warning is advisory and does not block play after homing. (3) Persisting the comparison reference remains an explicit `SAVE_SETTINGS` action, avoiding a flash erase/write and wear on every home.
- **Supersedes:** unbounded `HOME_SEEK_DRUM` / strike recovery motion and fault-clear behavior that could leave a stale homed calibration usable.
- **Affects:** `include/m2003_config.h`, `include/motor.h`, `include/strike.h`, `include/timing.h`, `src/motor.c`, `src/strike.c`, `src/protocol.c`, `src/timing.c`, `scripts/ring_bus.py`, `scripts/ring_tool.py`, `scripts/ring_midi_server.py`, `player/midi_player.html`, `protocol.md`, `midi_server_api.md`, and `docs/firmware.md`.

## 2026-08-13 — Operator features are complete only through the UI/server path

- **Decision:** Firmware behavior is not considered complete until its normal operator workflow is observable and actionable through the MIDI server and browser UI. Ordinary motor and strike tuning supports the same explicit target choices—one selected actuator or every enumerated actuator—and a fleet-applied value remains individually adjustable afterward. The ring command-line tool remains a diagnostic and exceptional-maintenance fallback, not a required step in routine tuning or fault recovery.
- **Why:** The instrument is normally operated through the browser and contains 14 or more actuators. Repeating commands manually is tedious and makes partial application or unnoticed faults more likely.
