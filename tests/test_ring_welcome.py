...
class WelcomeDelayTests(unittest.TestCase):
    ...
    def test_backlogged_greetings_have_one_second_gap(self):
        config = {
            "ip_range": "192.0.2.0/30",
            "interval_s": 25,
            "arrival_delay_s": 0,
            "people": [
                ...
                {
                    "name": "Grace",
                    "match": {"hostname": ["grace"]},
                    "song": {"builtin": "chirp"},
                },
            ],
        }
        devices = [welcome.Device("192.0.2.1"), welcome.Device("192.0.2.2")]
        devices[0].add_name("ada")
        devices[1].add_name("grace")
        clock = [100.0]
        plays = []
        ...
        with (
            ...
            patch.object(welcome, "load_state", return_value={}),
            patch.object(welcome, "save_state"),
            patch.object(welcome, "scan_network", return_value=devices),
            patch.object(welcome, "ChimeClient", FakeClient),
            patch.object(welcome, "play_person_song", side_effect=fake_play),
            patch.object(welcome.time, "monotonic", side_effect=lambda: clock[0]),
            patch.object(welcome.time, "sleep", side_effect=fake_sleep),
        ...
        self.assertEqual(result, 0)
        self.assertEqual(plays, [("Ada", 100.05), ("Grace", 101.05)])
